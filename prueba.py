...
columns_cartesian = ['frame_number']
...
columns_polar = ['frame_number']
for landmark in landmarks_of_interest:
    columns_polar.append(landmark.name + '_r')
    columns_polar.append(landmark.name + '_theta')
pose_data_polar = pd.DataFrame(columns=columns_polar)

pose = mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5)
video_writer = cv2.VideoWriter(output_video_path, cv2.VideoWriter_fourcc(*'mp4v'), 30, (int(cap.get(3)), int(cap.get(4))))
...
while cap.isOpened():
    ...
    pose_row_cartesian = {'frame_number': frame_number}
    if results.pose_landmarks:
        for landmark in landmarks_of_interest:
            pose_row_cartesian[landmark.name + '_x'] = results.pose_landmarks.landmark[landmark].x
            pose_row_cartesian[landmark.name + '_y'] = results.pose_landmarks.landmark[landmark].y
    else:
        for landmark in landmarks_of_interest:
    ...
    pose_row_polar = {'frame_number': frame_number}
    if results.pose_landmarks:
        for landmark in landmarks_of_interest:
            ...
            theta = np.arctan2(y, x)  # Calcular el ángulo
            pose_row_polar[landmark.name + '_r'] = r
            pose_row_polar[landmark.name + '_theta'] = theta
    else:
        for landmark in landmarks_of_interest:
            pose_row_polar[landmark.name + '_r'] = None
            pose_row_polar[landmark.name + '_theta'] = None
